...
(./bin/run.sh "crud" "test" "yarn test --verbose") || status=1
...
(./bin/run.sh "web-utils" "test" "yarn test --verbose") || status=1
...
(./bin/run.sh "web-api-token-auth-client" "test" "yarn test --verbose") || status=1
